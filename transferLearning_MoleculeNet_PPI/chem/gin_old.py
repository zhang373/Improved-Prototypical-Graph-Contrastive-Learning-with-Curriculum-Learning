    ...
    def get_embeddings(self, *argv):
        ...
        with torch.no_grad():
            for it, data in enumerate(loader):
                ...
                x, emb = self.forward(x, edge_index, batch, Encoder_support)

                # ret.append(x.cpu().numpy())
                ret.append(emb.cpu().numpy())
                print(data.y)
                y.append(data.y.view(emb.shape))
                #y.append(batch.y.view(ret.shape).to(torch.float64))
        ret = np.concatenate(ret, 0)
        y = np.concatenate(y, 0)
        #y = y.view(ret.shape).to(torch.float64)
        return ret, y
