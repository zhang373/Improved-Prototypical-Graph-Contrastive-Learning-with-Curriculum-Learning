...
class Encoder(torch.nn.Module):
    ...
    def get_embeddings(self, *argv):
        loader, device, Encoder_support = argv[0],argv[1], argv[2]
        #device = argv[2]#torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        for data in enumerate(loader):
            ...
            print(type(x))#,type(tensor(x)))
            # todo: 下边这里有问题,解决了，是因为上边的问题
            x, emb = self.forward(x, edge_index, batch, Encoder_support)     # emb就是embbeding的东西
        ...
        return ret ,y.view(ret.shape)#torch.from_numpy(ret), torch.from_numpy(y)   # 第1个有用， 第二个是干嘛的我有点没看懂。。。。
