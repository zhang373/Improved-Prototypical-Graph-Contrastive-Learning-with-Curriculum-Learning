...
def Set_num_task(args):
    ...
    # t_odo: 对于NCI1是二分类问题，因为用来SVM，我甚至觉得所有都是二分类问题，离谱
    return 2
...
def Train(config):
    ...
    Change3_BatchSize = config['Change3_BatchSize']
    ...
    for epoch in range(1, args.epoch+1):
        ...
        loss = get_loss(model, classifier, dataloader)
...
def main():
    ...
    config = {
        ...
        "Change1_lr": tune.sample_from(lambda spec: np.random.uniform(0.001, 0.2)),
        "Change2_hiddenDim_classifier": tune.choice([100, 130, 150, 170, 200, 220, 250, 270, 300]),  # 从给定值中随机选择
        "Change3_BatchSize": tune.choice([16, 32, 64, 128, 256]) # 自定义采样
    }
    ##print(config)
    #Train(config)
