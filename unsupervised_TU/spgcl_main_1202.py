...
class simclr(nn.Module):
    def __init__(self, hidden_dim, num_gc_layers, nmb_prototypes=0,dataset_num_features=1, alpha=0.5, beta=1., gamma=.1):
    ...
    def loss_cal(self, z, reliab_neg_mask,reliab_pos_mask,reliab_pos_idx,device):
        estimator = 'easy'
        temperature = 0.2
...
def get_div(p,q):
    ...
    #print("JS:", p,q)
    #M = (p+q) / 2
    #print(":JS:",0.5*scipy.stats.entropy(p, M),0.5*scipy.stats.entropy(q, M))
...
class encoder_support(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        return x
def train_spgcl(config,checkpoint_dir=None):
    ...
    aug = config['aug']
    prototype_num = config['prototype_num']
    Change4_num_chirality_tag, Change3_num_atom_type, dataset_num_features=\
        config['Change4_num_chirality_tag'], config['Change3_num_atom_type'], config['dataset_num_features']
    print("prototype_num:{}".format(prototype_num))
    ...
    #dataset = TUDataset(path, name=DS, aug=aug,stro_aug='stro_dnodes').shuffle()
    print("dataset/" + DS)
    dataset= MoleculeDataset_aug("dataset/" + DS, dataset=DS)
    # dataset_eval = TUDataset(path, name=DS, aug='none',stro_aug='none').shuffle()
    dataset_eval = MoleculeDataset_aug("dataset/" + DS, dataset=DS)
    print(len(dataset))
    ...
    model = simclr(hidden_dim, num_gc_layers, nmb_prototypes=prototype_num,dataset_num_features=dataset_num_features).to(device)
    ...
    for epoch in range(1, epochs+1):
        ...
        # enumerate(tqdm(loader, desc="Iteration"))
        for it, data in enumerate(dataloader):
            # measure data loading time
            data_time.update(time.time() - end)
            data, data_aug, data_stro_aug = data, data, data
            print(type(data))
            optimizer.zero_grad()
            ...
            # ============ forward passes ... ============
            # feature, scores
            embedding, prot_scores1 = model(Encoder_support.forward(data.x), data.edge_index, data.batch, data.num_graphs)
            print(data.y)
            #y = data.y.view(embedding.shape).to(torch.float64)
            bs = batch_size #data.y.size(0)
            # print(f"embedding: {embedding}")
            # print(f"model.prototypes: {model.prototypes}")
            
            # print(model.prototypes.weight.size())

            if aug == 'dnodes' or aug == 'subgraph' or aug == \
                    'ppr_aug' or aug == 'random2' or aug == 'random3' \
                    or aug == 'random4' or aug == 'dedge_nodes':
            ...
            _embedding, prot_scores2 = model(Encoder_support.forward(data_aug.x), data_aug.edge_index, data_aug.batch, data_aug.num_graphs)
            ...
            sample_num = 2*bs
            reliab_num = reliab_pacing(info_ent_avg,config['info_ent_threshold'],config['reliab_pacing_type'],sample_num,epoch,epochs)
            ...
            #print("pos_div:",pos_div)
            #print("pos_cand_mask:", pos_cand_mask)
            pos_cand_div_avg = torch.sum(pos_div * pos_cand_mask) / pos_cand_num
            #print("up:",torch.sum(pos_div * pos_cand_mask),"down:",pos_cand_num)
            print("pos_cand_div_avg",pos_cand_div_avg)
            reliab_pos_num = reliab_pacing(pos_cand_div_avg,config['pos_div_threshold'],config['pos_reliab_pacing_type'],pos_cand_num,epoch,epochs)
            ...
            reliab_neg_num = reliab_neg_pacing(neg_cand_div_avg,config['neg_div_threshold'],config['neg_reliab_pacing_type'],neg_cand_num,epoch,epochs)
            reliab_neg_mask,reliab_neg_row_idx,reliab_neg_col_idx = get_reliab_neg_mask(neg_div,reliab_neg_num)
            #计算对比损失
            
            L_contrastive = model.loss_cal(z,reliab_neg_mask,reliab_pos_mask,reliab_pos_idx,device)
            ...
            # import pdb
            # pdb.set_trace()
            loss += L_contrastive + config['lamda1'] * L_cluster_consistency + config['lamda2'] * L_cluster_reg
            # print(f"loss:{loss}")
            # import pdb
            # pdb.set_trace()
        ...
        tune.report(my_loss = loss_all / len(dataloader))
    
    #print('[info] MAX acc:{}'.format(max_acc))
    tune.report(Max_acc = max(accuracies['test']))
if __name__ == '__main__':
    ray.init()
    device = torch.device("cuda:1")
    config = {
        "lr":0.01,#tune.choice([0.01,0.005,0.001,0.0005,0.0001]),
        "DS":'zinc_standard_agent',
        "aug":'dnodes',
        "batch_size":128,#tune.choice([64,128,256,512]),
        "pos_div_threshold":0.5,#tune.uniform(0,1),
        "neg_div_threshold":0.5,#tune.uniform(0,1),
        "prototype_num":10,#,tune.choice([5,10,15,20,30,40,50]),
        "info_ent_threshold":2.5,#tune.uniform(0,5.6),
        "lamda1":0.5,#tune.uniform(0,1),
        "lamda2":0.5,#tune.uniform(0,1),
        "neg_reliab_pacing_type":'logar',
        "pos_reliab_pacing_type":'logar',
        "reliab_pacing_type":'logar',
        "hidden_dim":128,#tune.choice([32,64,128,256,512]),
        "num_gc_layers":3,#tune.choice([2,3,4,5]),
        "device":device ,
        "Change4_num_chirality_tag": 3,
        "Change3_num_atom_type": 128,
        "dataset_num_features": 96
    }
    train_spgcl(config)
    #result = tune.run(
    #    train_spgcl,
